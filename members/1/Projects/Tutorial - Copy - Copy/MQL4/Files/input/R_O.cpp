...
//--------------END ACTIONS ------------------------------
    ...
    Set_Rule(OP_BUYSELL, T_BUYLOTSL, R_O, P_SIGNAL, ((RValue (OP_BUYSELL, T_LASTORDEROPENPRICE, R_O) - SValue(DOWNFRACTAL, S_CURRENT, P_H4)) / Point));
    Set_Rule(OP_BUYSELL, T_SELLLOTSL, R_O, P_SIGNAL, ((SValue(UPFRACTAL, S_CURRENT, P_H4) - RValue (OP_BUYSELL, T_LASTORDEROPENPRICE, R_O)) / Point));
    if ((AndS(UPFRACTAL, S_ABOVE, P_H4) && (RValue (OP_BUYSELL, T_LASTORDERTYPE, R_O) == OP_SELL)))
    {
        Set_Rule(OP_BUY, T_STATUS, R_O, P_SIGNAL);
    }
    if ((AndS(DOWNFRACTAL, S_BELOW, P_H4) && (RValue (OP_BUYSELL, T_LASTORDERTYPE, R_O) == OP_BUY)))
    {
        Set_Rule(OP_SELL, T_STATUS, R_O, P_SIGNAL);
    }
}
