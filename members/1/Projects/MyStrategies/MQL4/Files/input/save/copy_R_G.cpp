...
//--------------PROPERTIES ------------------------------
    ...
    Set_Rule(OP_BUYSELL, T_RECOVERYVALUE, R_G, P_SIGNAL, 20);
    Set_Rule(OP_BUYSELL, T_ILOT, R_G, P_SIGNAL, 0.01);
    Set_Rule(OP_BUYSELL, T_MAXLOT, R_G, P_SIGNAL, 10);
    Set_Rule(OP_BUYSELL, T_MAXCOUNT, R_G, P_SIGNAL, 30);
    Set_Rule(OP_BUYSELL, T_KEEPBUYSELL, R_G, P_SIGNAL, false);
    Set_Rule(OP_BUYSELL, T_EXITMODE, R_G, P_SIGNAL, EM_EXITANY);
    Set_Rule(OP_BUYSELL, T_PIPSTEP, R_G, P_SIGNAL, -1);
    Set_Rule(OP_BUYSELL, T_TIMESTEP, R_G, P_SIGNAL, 5);
    ...
    Set_Rule(OP_EXIT, T_MINPROFIT, R_G, P_SIGNAL, 10);
//--------------END PROPERTIES ------------------------------
//
//
//--------------ACTIONS ------------------------------
...
//--------------END ACTIONS ------------------------------
    if ((((SValue(LOW, S_CURRENT, P_M1) > SValue(EMA_34, S_CURRENT, P_M1)) && (SValue(LOW, S_CURRENT, P_H1) > SValue(EMA_34, S_CURRENT, P_H1))) && (SValue(EMA_34, S_ANGLE, P_M1) > 15) && (SValue(EMA_34, S_ANGLE, P_H4) > 10)))
    {
        ...
        if (((RValue (OP_BUYSELL, T_LASTORDERTYPE, R_G) == OP_SELL) && (SValue(EMA_34, S_ANGLE, P_H4) > -15)))
        {
            Set_Rule(OP_BUYSELL, T_RECOVERYMODE, R_G, P_SIGNAL, M_K);
            Set_Rule(OP_BUYSELL, T_MAXCOUNT, R_G, P_SIGNAL, (RValue (OP_BUYSELL, T_MAXCOUNT, R_G) + 1));
        }
    }
    if ((((SValue(HIGH, S_CURRENT, P_M1) < SValue(EMA_34, S_CURRENT, P_M1)) && (SValue(HIGH, S_CURRENT, P_H1) < SValue(EMA_34, S_CURRENT, P_H1))) && (SValue(EMA_34, S_ANGLE, P_M1) < -15) && (SValue(EMA_34, S_ANGLE, P_H4) < -10)))
    {
        ...
        if (((RValue (OP_BUYSELL, T_LASTORDERTYPE, R_G) == OP_BUY) && (SValue(EMA_34, S_ANGLE, P_H4) < -10)))
        {
            Set_Rule(OP_BUYSELL, T_RECOVERYMODE, R_G, P_SIGNAL, M_K);
            Set_Rule(OP_BUYSELL, T_MAXCOUNT, R_G, P_SIGNAL, (RValue (OP_BUYSELL, T_MAXCOUNT, R_G) + 1));
        }
