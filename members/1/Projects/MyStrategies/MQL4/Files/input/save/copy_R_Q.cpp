...
/* ============================================================
STRATEGY : PivotPoint  Cross Line
RULE : Q
DESCRIPTION
...
//--------------PROPERTIES ------------------------------
    ...
    Set_Rule(OP_EXIT, T_MINPROFIT, R_Q, P_SIGNAL, 100);
//--------------END PROPERTIES ------------------------------
//
//
//--------------ACTIONS ------------------------------
...
//--------------END ACTIONS ------------------------------
    ...
    if ((AndS(PIVOT_POINT, S_BELOW, P_D1) && (AndPS(BAR, S_BULL_HAMMER, P_D1) || AndPS(BAR, S_BULL_HAMMER1, P_D1) || AndPS(BAR, S_BULL_HAMMER2, P_D1) || AndPS(BAR, S_BULL, P_D1)) && !AndS(PIVOT_POINT, S_CROSS_UP, P_D1) && !AndS(PIVOT_POINT, S_CROSS_DOWN, P_D1) && (Distance_From_Pivot <= 15)))
    {
        Set_Rule(OP_BUYSELL, T_START, R_Q, P_SIGNAL);
        Set_Rule(OP_BUY, T_STATUS, R_Q, P_SIGNAL);
    }
    if ((AndS(PIVOT_POINT, S_ABOVE, P_D1) && (AndPS(BAR, S_BEAR_SHOOTING_STAR, P_D1) || AndPS(BAR, S_BEAR_SHOOTING_STAR1, P_D1) || AndPS(BAR, S_BEAR_SHOOTING_STAR2, P_D1) || AndPS(BAR, S_BEAR, P_D1)) && !AndS(PIVOT_POINT, S_CROSS_DOWN, P_D1) && !AndS(PIVOT_POINT, S_CROSS_UP, P_D1) && (Distance_From_Pivot <= 15)))
    {
        Set_Rule(OP_BUYSELL, T_START, R_Q, P_SIGNAL);
