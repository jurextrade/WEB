...
//--------------PROPERTIES ------------------------------
    ...
    Set_Rule(OP_EXIT, T_MINPROFIT, R_G, P_SIGNAL, 100);
//--------------END PROPERTIES ------------------------------
//
//
//--------------ACTIONS ------------------------------
...
//--------------END ACTIONS ------------------------------
    if ((AndS(RSI_2, S_OVERSOLD, P_H1) && AndS(MA_200, S_ABOVE, P_H1)))
    {
        Set_Rule(OP_BUYSELL, T_START, R_G, P_SIGNAL);
        Set_Rule(OP_BUY, T_STATUS, R_G, P_SIGNAL);
    }
    if ((AndS(RSI_2, S_OVERBOUGHT, P_H1) && AndS(MA_200, S_BELOW, P_H1)))
    {
        Set_Rule(OP_BUYSELL, T_START, R_G, P_SIGNAL);
