...
//
//--------------PROPERTIES ------------------------------
    Set_Rule(OP_BUYSELL, T_DIRECTION, R_Y, P_SIGNAL, D_ANY);
    Set_Rule(OP_BUYSELL, T_DIRECTIONTYPE, R_Y, P_SIGNAL, DT_LEVEL);
    Set_Rule(OP_BUYSELL, T_RECOVERYMODE, R_Y, P_SIGNAL, M_C);
    Set_Rule(OP_BUYSELL, T_RECOVERYVALUE, R_Y, P_SIGNAL, 1);
    Set_Rule(OP_BUYSELL, T_ILOT, R_Y, P_SIGNAL, 0.1);
    Set_Rule(OP_BUYSELL, T_MAXLOT, R_Y, P_SIGNAL, 1);
    Set_Rule(OP_BUYSELL, T_MAXCOUNT, R_Y, P_SIGNAL, 1);
    Set_Rule(OP_BUYSELL, T_KEEPBUYSELL, R_Y, P_SIGNAL, false);
    Set_Rule(OP_BUYSELL, T_EXITMODE, R_Y, P_SIGNAL, EM_EXITANY);
    Set_Rule(OP_BUYSELL, T_PIPSTEP, R_Y, P_SIGNAL, 10);
    Set_Rule(OP_BUYSELL, T_TIMESTEP, R_Y, P_SIGNAL, 10);
    Set_Rule(OP_BUYSELL, T_MAXTIME, R_Y, P_SIGNAL, -1);
    Set_Rule(OP_BUYSELL, T_HEDGEMAGNITUDE, R_Y, P_SIGNAL, 1);
    Set_Rule(OP_BUYSELL, T_BUYTS, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_BUYTP, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_BUYSL, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_SELLTS, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_SELLTP, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_SELLSL, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_TS, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_TP, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_SL, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_BUYLOTTS, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_BUYLOTTP, R_Y, P_SIGNAL, 5);
    Set_Rule(OP_BUYSELL, T_BUYLOTSL, R_Y, P_SIGNAL, 5);
    Set_Rule(OP_EXIT_BUY, T_MINPROFIT, R_Y, P_SIGNAL, -1);
    Set_Rule(OP_BUYSELL, T_SELLLOTTS, R_Y, P_SIGNAL, 0);
    Set_Rule(OP_BUYSELL, T_SELLLOTTP, R_Y, P_SIGNAL, 5);
    Set_Rule(OP_BUYSELL, T_SELLLOTSL, R_Y, P_SIGNAL, 5);
    Set_Rule(OP_EXIT_SELL, T_MINPROFIT, R_Y, P_SIGNAL, -1);
    Set_Rule(OP_EXIT, T_MINPROFIT, R_Y, P_SIGNAL, -1);
//--------------END PROPERTIES ------------------------------
//
//
//--------------ACTIONS ------------------------------
    if (Trade_no (OP_BUYSELL, R_Y))
    {
        Set_Rule(OP_EXIT, T_STATUS, R_Y, P_SIGNAL);
    }
//--------------END ACTIONS ------------------------------
    if (AndS(STO_SAR, S_SELL, P_M15))
    {
        Set_Rule(OP_BUYSELL, T_START, R_Y, P_SIGNAL);
        Set_Rule(OP_SELL, T_STATUS, R_Y, P_SIGNAL);
    }
    if (AndS(STO_SAR, S_BUY, P_M15))
    {
        Set_Rule(OP_BUYSELL, T_START, R_Y, P_SIGNAL);
        Set_Rule(OP_BUY, T_STATUS, R_Y, P_SIGNAL);
    }
}
