...
//--------------PROPERTIES ------------------------------
    ...
    Set_Rule(OP_BUYSELL, T_DIRECTIONTYPE, R_H, P_SIGNAL, DT_LEVEL);
    ...
    Set_Rule(OP_BUYSELL, T_BUYLOTTS, R_H, P_SIGNAL, 10);
    Set_Rule(OP_BUYSELL, T_BUYLOTTP, R_H, P_SIGNAL, 20);
    Set_Rule(OP_BUYSELL, T_BUYLOTSL, R_H, P_SIGNAL, 20);
    Set_Rule(OP_EXIT_BUY, T_MINPROFIT, R_H, P_SIGNAL, -1);
    Set_Rule(OP_BUYSELL, T_SELLLOTTS, R_H, P_SIGNAL, 10);
    Set_Rule(OP_BUYSELL, T_SELLLOTTP, R_H, P_SIGNAL, 20);
    Set_Rule(OP_BUYSELL, T_SELLLOTSL, R_H, P_SIGNAL, 20);
    Set_Rule(OP_EXIT_SELL, T_MINPROFIT, R_H, P_SIGNAL, -1);
    Set_Rule(OP_EXIT, T_MINPROFIT, R_H, P_SIGNAL, 20);
//--------------END PROPERTIES ------------------------------
//
//
//--------------ACTIONS ------------------------------
...
//--------------END ACTIONS ------------------------------
    if ((!Trade_Buy_OnBar_H1 (OP_BUYSELL, R_H) && AndS(Singapore_Swing, S_SELL, P_H1)))
    {
        Set_Rule(OP_BUYSELL, T_START, R_H, P_SIGNAL);
        Set_Rule(OP_SELL, T_STATUS, R_H, P_SIGNAL);
    }
    if ((!Trade_Sell_OnBar_H1 (OP_BUYSELL, R_H) && AndS(Singapore_Swing, S_BUY, P_H1)))
    {
        Set_Rule(OP_BUYSELL, T_START, R_H, P_SIGNAL);
