function project_home_init () {
   // InitEMVAssistant();
}

//----------------------------------------------------   CAROUSEL PANEL    ------------------------------------------------   

function project_MainCarouselPanel () {
    var content = 
        sb.render (project_assistant_panel) +     
'       <div id="project_maincarousel" class="featured carousel slide sb_panel" data-bs-ride="carousel">' +
...
function project_PresentationPanel () {
    var content =    
...
function project_home_open () {
    if (!sb.tab_finditem(strategyfiletab, 'project_home_tab')) { 
        let hometabitem  = {id: 'project_home_tab',   item: 'Home',      type:'link', in:  icon_home,       items: [project_home_main], onclose: 'onclick="onclick_StrategyCloseTabItem(this, event)"',   title: 'Home',               events: {onclick:"onclick_project_tab(this)"}};

        sb.tab_additem(strategyfiletab, hometabitem, 1);
        project_home_init();        
    } 
    sb.tab_select(strategyfiletab, 'project_home_tab');
}
...
function onclick_connectMT4Terminal1 (elt, event) { 
...
function onclick_optionstutorial1(elt, event) {
    if (!$('#tradedesk_assistantpanel').is(':hidden')) {
        MT4AssistantPanel_Close ();    
    }
...
function project_GetStartedSection_Panel () {
    var content = 
...
function project_ToolsSection_Panel () {
    var content = 
...
function projectslideToggleCallback () {
    sb.resize(body)
}
...
function project_AssistantPanel_Close() {
    assistant_login = false;    
    sb.toggle ($('#project_maincarousel'), 1);
    sb.toggle ($('#project_assistantpanel'), 0, projectslideToggleCallback);
}

function project_AssistantPanel_Open() {
    sb.toggle ($('#project_maincarousel'), 0);
    sb.toggle ($('#project_assistantpanel'),1, projectslideToggleCallback);
}

function project_assistant_register () {
    assistant_login = true;
    Register();
}
 

function onclick_project_assistantclose(elt) {
    project_AssistantPanel_Close();  
}




function project_AssistantPanel () {
     var content = 
...
'       <form  id="project_assistantform"  class="assistant_panel">' +
'           <div id="loginclose" onclick="onclick_project_assistantclose(this)" ><svg class="installcheckmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 52 52"><path class="checkmark__check" fill="none" d="M16 16 36 36 M36 16 16 36"></path></svg></div>   ' +  
...
'                                   <button class="assistant_button sb_button noactivate" title="" type="button" onclick="project_assistant_register()"><i class="icon_mt4expert"></i>Login or Register</button>' +
'                               </div>' +
'                           </div>' +
'                       </div>' +
'                   </div>' +
