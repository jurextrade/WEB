...
#define SYSTEM(X) SetSystemObjects (int x, double Ask, double Bid, datetime CurrentTime, int CurrentPeriod, std::string Symbol, double Point, double Digits, \
...
#define R_RULE(x)	R_ ## x ## _RULE (double Ask, double Bid, datetime CurrentTime, int CurrentPeriod, std::string Symbol, double Point, double Digits, \
									  int* SignalTab, double* SignalTabValue, datetime*	SignalTabTime, double* SignalTabPrice, \
									  int* BeforeSignalTab, double* BeforeSignalTabValue, datetime* BeforeSignalTabTime, double* BeforeSignalTabPrice,\
									  int* BeforeSignalTickTab, int* RuleTab, double* RuleTabValue, int* BeforeRuleTab, double* BeforeRuleTabValue)
...
#define Set_Signal(...) iSet_Signal (SignalTab, BeforeSignalTickTab, SignalTabValue, SignalTabTime, SignalTabPrice, CurrentTime, Bid, ##__VA_ARGS__)  
...
extern void iSet_Signal (int* SignalTab, int* BeforeSignalTickTab, double* SignalTabValue, datetime* SignalTabTime, double* SignalTabPrice, datetime CurrentTime, double Bid, int Object, int SignalType, int period, int signal,double value = -1);
...
#define ReturnTime(stime) _ReturnTime (CurrentTime, stime)
#define Pips(value) 	  _Pips (value, Point)
#define PipValue(pips) 	  _PipValue (pips, Point)
extern datetime _ReturnTime (datetime CurrentTime, const char sTime[]);
extern datetime ReturnDate (const char sDate[]);

extern double _Pips (double value, double point); 
extern double _PipValue (double pips, double point); 

extern int GetMonth (datetime CurrentTime);
extern int GetWeek (datetime CurrentTime);
extern int GetDay (datetime CurrentTime); 
extern int GetHours (datetime CurrentTime); 
extern int GetMinutes (datetime CurrentTime); 
