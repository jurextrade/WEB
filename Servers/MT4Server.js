...
function SendFileExpert (userid, fromfile, filename, filetype, projectfolder, socket, emplacement) {                //http
    ...
    form.append('projectfolder', 'projectfolder');
 //   form.append('content', new ArrayBuffer(content));
    ...
    fetch(url, {
                ...
                body: '?user_id =' + userid + '&filename=' + filename + '&projectfolder=' + projectfolder + '&emplacement=' + emplacement,
            })
...
/*
    ...
    http.onload = function () {
        ...
        if (http.status === 200) {
            var strerror = "UPLOAD " +  fromfile + " to " + filename  + " OK successfully uploaded\n";
            Print(strerror);
            NS.SendToHTTPClient(socket, ":______**UPLOAD^" + fromfile   +  "^" + projectfolder  +  "^" + filetype +  "^OK^"  + strerror + "*");   	
        } else {
            var strerror = "UPLOAD " +  fromfile + " to " +    tofile  + " Failed\n";    
            Print(strerror);
            NS.SendToHTTPClient(socket, ":______**UPLOAD^" + fromfile   +  "^" + projectfolder  +  "^" + filetype + "^ERROR^" + strerror  + "*");              
        }
    };
    http.send('?user_id =' + userid + '&filename=' + filename + '&projectfolder=' + projectfolder + '&emplacement=' + emplacement);
*/    
}
